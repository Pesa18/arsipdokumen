...
class Chart extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->session->tipe == "admin" || $this->session->tipe == "operator") {
			...
			$query_chart3 =  $this->db->query("SELECT COUNT(id) as count,MONTHNAME(tgl_input) as month_name FROM data_arsip WHERE YEAR(tgl_input) = '" . date('Y') . "' and username='".$_SESSION['username']."' GROUP BY YEAR(tgl_input),MONTH(tgl_input)"); 
		}
